...
class CustomerGenerator extends Generator
{
    ...
    {
        $namespace = $vendorName . '\\' . ProjectGenerator::PROJECT . '\\' . $customerName . '\\' . self::BUNDLE;
        ...
        $basename = substr($vendorName . ProjectGenerator::PROJECT . $customerName . self::BUNDLE, 0, -6);
        $parameters = array(
            'namespace' => $namespace,
            'bundle'    => self::BUNDLE,
            'format'    => 'yml',
        ...
        $this->filesystem->mkdir($targetDir . '/' . $vendorName . '/' . '/' . ProjectGenerator::PROJECT . '/' . '/' . $customerName . '/' . self::SITES);
    }
}